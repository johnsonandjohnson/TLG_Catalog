...
adsl <- adsl %>%
  filter(.data[[popfl]] == "Y") %>%
  select(USUBJID, starts_with("TRT01"), all_of(popfls), all_of(unique(c(demog_vars, demog_displ_vars))))
...
adsl$REGION1 <- factor(as.character(adsl$REGION1), levels = sort(unique(as.character(adsl$REGION1))))
...
lyt <- basic_table(
  ...
  split_rows_by("REGION1", split_label = "Region", split_fun = trim_levels_in_group("COUNTRY_DECODE"), label_pos = "topleft", section_div = " ") %>%
  summarize_row_groups("REGION1") %>%
  split_rows_by("COUNTRY_DECODE", split_label = "Country/Territory", split_fun = trim_levels_in_group("SITEID"), label_pos = "topleft", section_div = " ") %>%
