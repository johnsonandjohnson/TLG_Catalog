...
lbtoxgrade_defs <- unique(unique(lbtoxgrade_defs %>%
...
ad_toxterms <- bind_rows(
  unique(adlb_complete %>% select(PARAMCD, ATOXDSCL) %>% mutate(TOXTERM = ATOXDSCL, TOXDIR = "LOW")),
  unique(adlb_complete %>% select(PARAMCD, ATOXDSCH) %>% mutate(TOXTERM = ATOXDSCH, TOXDIR = "HIGH"))
) %>%
...
adlb00 <- adlb_complete %>%
  ...
  relocate(., USUBJID, ANL04FL, ANL05FL, ONTRTFL, TRTEMFL, AVISIT, ATOXGRL, ATOXGRH, ATOXDSCL, ATOXDSCH, PARAMCD, AVISIT, AVAL)
...
filtered_adlb_low <- filtered_adlb %>%
  ...
  select(USUBJID, starts_with("PAR"), starts_with("ATOX"), all_of(flagvars), ANL04FL, ANL05FL, ANL045FL, LBSEQ, AVAL, AVALC) %>%
...
xx <- adlb00 %>% filter(ATOXDSCL == "Neutrophil Count Decreased" & !is.na(ATOXGRL))
...
lyt <- basic_table(show_colcounts = TRUE, colcount_format = "N=xx") %>%
  ...
  split_rows_by("ATOXDSCLH",
    label_pos = "topleft",
    split_label = paste0("Laboratory Test \u2265 Grade ", grade_threshold, ", n (%)"),
    child_labels = "hidden",
    split_fun = drop_split_levels
  ) %>%
  analyze("ATOXGRLHx",
