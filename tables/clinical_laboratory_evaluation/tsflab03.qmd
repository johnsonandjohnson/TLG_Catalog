---
title: TSFLAB03
subtitle: Subjects With ≥1 Laboratory Values With Elevated or Low Values Based on Worst On-treatment Value Using NCI-CTCAE Criteria
---
...
availparcat56 <- c("Investigations", "Metabolism and nutritional disorders", "Renal and urinary disorders", "Blood and lymphatic system disorders")
...
lbtoxgrade_defs <- unique(lbtoxgrade_defs %>%
...
adlb00 <- adlb_complete %>%
  select(
    USUBJID, AVISITN, AVISIT, starts_with("PAR"), starts_with("ATOX"), starts_with("ANL"),
    ONTRTFL, TRTEMFL, AVAL, APOBLFL, ABLFL, LVOTFL
  ) %>%
  inner_join(adsl) %>%
  mutate(
  ...
  relocate(., USUBJID, ANL04FL, ANL05FL, ONTRTFL, TRTEMFL, AVISIT, ATOXGRL, ATOXGRH, ATOXDSCL, ATOXDSCH, PARAMCD, AVISIT, AVAL, APOBLFL, ABLFL)
...
parcat <- unique(adlb00 %>% select(starts_with("PARCAT"), PARAMCD, PARAM, ATOXDSCL, ATOXDSCH) %>%
...
filtered_adlb_tox <-
  ...
  mutate(PARCAT56 = factor(PARCAT56,
    levels = unique(c("Blood and lymphatic system disorders", levels(adlb_complete$PARCAT5)))
  ))
...
if (nrow(check_non_unique_subject)) message("Please review your data selection process, subject has multiple records")



params <- unique(filtered_adlb_tox %>% select(PARCAT56, PARAMCD, PARAM, ATOXDSCLH, ATOXDIR))

all_params <- unique(adlb_complete %>%
  filter(!(is.na(PARCAT5) & is.na(PARCAT6))) %>%
...
lbtoxgrade_defs <- lbtoxgrade_defs %>%
  inner_join(
    .,
    unique(filtered_adlb_tox %>% select(PARAMCD, PARAM, ATOXDIR, ATOXDSCLH, PARCAT5, PARCAT6, PARCAT56)),
...
extra_args_rr2 <- list(method = "wald", denom = "n_df", ref_path = ref_path, .stats = c("denom","count_unique_denom_fraction"))  
