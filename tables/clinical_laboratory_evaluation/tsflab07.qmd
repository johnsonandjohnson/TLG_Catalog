...
availparcat56 <- c("Blood and lymphatic system disorders", "Investigations", "Metabolism and nutritional disorders", "Renal and urinary disorders")
...
adlb00 <- adlb_complete %>%
  ...
  select(
    ...
    PARCAT1, PARCAT3, PARCAT5, PARCAT6,
    starts_with("ATOX"),
    starts_with("BTOX"),
    starts_with("ANL"),
    all_of(flagvars),
  ...
  inner_join(adsl) %>%
  relocate(., USUBJID, ANL04FL, ANL05FL, ONTRTFL, TRTEMFL, AVISIT, ATOXGRL, ATOXGRH, ATOXDSCL, ATOXDSCH, ABLFL, BTOXGRL, BTOXGRH, PARAMCD, PARCAT3, AVISIT, AVAL)
...
filtered_adlb_tox <-
  ...
  mutate(
    ...
    BTOXGRLH = factor(paste("Grade", BTOXGRLH), levels = c("N", paste("Grade", 0:4)))
  )
...
check_categories <- unique(filtered_adlb_tox %>% select(PARCAT3, PARCAT56, ATOXDSCLH)) %>%
...
lyt <- basic_table(show_colcounts = FALSE) %>%
  ...
  analyze("ATOXGRLH",
          ...
          extra_args = list(
            .stats = "count_unique",
            denom = "n_rowdf",
            new_levels = list(c("Total"), list(c("Grade 0", "Grade 1", "Grade 2", "Grade 3", "Grade 4"))),
