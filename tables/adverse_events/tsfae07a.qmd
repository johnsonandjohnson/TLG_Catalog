...
adae <- pharmaverseadamjnj::adae %>%
  ...
  select(USUBJID, TRTEMFL, AEBODSYS, AEDECOD, AESEV, all_of(special_interest_var), AESER, AEOUT, all_of(aeactionvar), all_of(aerelvar))
...
ae <- ae %>%
  mutate(
    ASEV = factor(ifelse(is.na(AESEV), "Missing", as.character(AESEV)),
...
lyt <- lyt %>%
  ...
  analyze("ASEV", afun = a_freq_j, indent_mod = 0, extra_args = append(extra_args_rr, NULL)) %>%
  split_rows_by("AESER",
