...
adaemax <- left_join(adaemaxpt, adaemaxsoc, by = c("USUBJID", "AEBODSYS", "AESEV"))
...
adaeall <- bind_rows(adaemax, adaetot) %>%
  mutate(ASEV = factor(as.character(AESEV),
...
ae$spanheader <- factor(ifelse(ae$ASEV == "Total", " ", "Severity"),
  levels = c(" ", "Severity")
)
...
adsl$spanheader <- factor(ifelse(adsl$ASEV == "Total", " ", "Severity"),
  levels = c(" ", "Severity")
)
...
if (length(adae$TRTEMFL) != 0) {
  result <- sort_at_path(result, c("root", "AEBODSYS"),
    scorefun = jj_complex_scorefun(colpath = c("colspan_trt", "Active Study Agent", trtvar, "Combined", "spanheader", " ", "ASEV", "Total"))
  )
...
tt_to_tlgrtf(string_map = string_map, tt = head(result, 20), file = fileid, orientation = "portrait", nosplitin = list(cols = c(trtvar)))
```
