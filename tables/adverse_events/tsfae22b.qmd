---
title: TSFAE22B
subtitle: Subjects With Treatment-emergent Adverse Events by System Organ Class, Preferred Term, and Sex
---
...
# Original Reporting Effort  : JNJxxxxxx\
...
adsl$spanheader <- factor(ifelse(adsl$SEXcat == "Total", " ", "Sex"),
                          levels = c(" ", "Sex")
)
...
if (length(adae$TRTEMFL) != 0) {
  result <- sort_at_path(result, c("root", "AEBODSYS"),
                         scorefun = jj_complex_scorefun(colpath = c("colspan_trt", "Active Study Agent", trtvar, "Combined", "spanheader", " ", "SEXcat", "Total"))
  )
...
tt_to_tlgrtf(string_map = string_map, tt = head(result, 20),
             file = fileid, orientation = "portrait", label_width_ins = 1.5,
