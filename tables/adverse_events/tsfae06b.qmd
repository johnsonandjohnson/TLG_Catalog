---
title: TSFAE06B
subtitle: Subjects With Treatment-emergent Adverse Events With Frequency ≥5% in Drug-treated Group and ≥2% More Often in Drug-treated Group Than Placebo-treated Group by Preferred Term
---
...
lyt <- lyt %>%
  analyze("AEDECOD", afun = a_freq_j, extra_args = append(extra_args_rr, NULL), indent_mod = 0L) %>%
...
if (length(adae$TRTEMFL) != 0) {
  ...
  result <- safe_prune_table(result, prune_func = bspt_pruner(
    fraction = 0.05, diff_from_control = 0.02,
    control = ctrl_grp
  ))
}
