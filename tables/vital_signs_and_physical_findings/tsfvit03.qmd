...
filtered_advs <- pharmaverseadamjnj::advs %>%
  ...
  filter(ANL06FL == "Y") %>%
  select(STUDYID, USUBJID, PARAMCD, PARAM, AVALCAT1, AVALCA1N, AVISIT, ANL06FL, APOBLFL, ONTRTFL) %>%
  inner_join(adsl)
...
lbmarkedlyabnormal_defs <- readxl::read_excel(markedlyabnormal_file, sheet = "ADVS") %>%
...
if (class(filtered_advs$AVALCAT1) == "character" ||
  !all(unique(param_map$AVALCAT1) %in% levels(filtered_advs$AVALCAT1))) {
  filtered_advs$AVALCAT1 <- factor(as.character(filtered_advs$AVALCAT1), levels = unique(param_map$AVALCAT1))
}
