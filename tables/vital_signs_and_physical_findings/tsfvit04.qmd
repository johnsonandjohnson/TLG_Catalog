...
filtered_advs <- pharmaverseadamjnj::advs %>%
  ...
  filter(ONTRTFL == "Y") %>%
  select(
    STUDYID, USUBJID, PARAMCD, PARAM, AVALCAT1, AVALCA1N, AVISIT, APOBLFL, CRIT3, CRIT3FL,
    ONTRTFL
  ) %>%
  inner_join(adsl) %>%
...
extra_args_rr <- list(denom = "n_df", riskdiff = TRUE, method = "wald", ref_path = ref_path,
...
lyt <- basic_table(
  ...
  analyze("CRIT3FL",a_freq_j, show_labels = "hidden", table_names = "CRIT3_N",
