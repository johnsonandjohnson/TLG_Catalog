---
title: TSFVIT05
subtitle: Subjects With On-treatment Clinically Important Vital Signs Over Time
---
...
sel_param <- c("Pulse Rate (beats/min)", "Systolic Blood Pressure (mmHg)", "Diastolic Blood Pressure (mmHg)")
sel_param_case <- c("Pulse rate (beats/min)", "Systolic blood pressure (mmHg)", "Diastolic blood pressure (mmHg)")
...
if (over_time) {
  ...
  filtered_advs_1 <- pharmaverseadamjnj::advs %>%
    ...
    filter(ONTRTFL == "Y") %>%
    select(
      STUDYID, USUBJID, PARAMCD, PARAM, AVALCAT1, AVALCA1N, AVISIT,
      ONTRTFL,
      APOBLFL, CRIT1, CRIT1FL, CRIT2, CRIT2FL, TRTEMFL, ANL02FL, AVAL
    ) %>%
    inner_join(adsl)
}
...
if (worst) {
  ...
  filtered_advs_2 <- pharmaverseadamjnj::advs %>%
    ...
    select(STUDYID, USUBJID, PARAMCD, PARAM, AVALCAT1, AVALCA1N, AVISIT, APOBLFL, CRIT1, CRIT1FL, CRIT2, CRIT2FL, TRTEMFL, ANL03FL, ANL06FL, AVAL) %>%
    inner_join(adsl)
...
if (over_time & worst) {
  ...
  filtered_advs$AVISIT <- factor(as.character(filtered_advs$AVISIT),
...
filtered_advs <- var_relabel(filtered_advs,
  PARAM = "Blood Pressure (mmHg)"
)
...
lyt <- lyt %>%
  ...
  analyze("CRIT1FL", a_freq_j, show_labels = "hidden", table_names = "CRIT1_N",
          extra_args = list(denom = "n_df", .stats = c("n_df")))  %>%
  analyze(c("CRIT1FL", "CRIT2FL"), a_freq_j,
    extra_args = append(
      extra_args_rr,
