...
sel_param_case <- c("Systolic blood pressure (mmHg)", "Diastolic blood pressure (mmHg)")
...
advs00 <- pharmaverseadamjnj::advs %>%
  select(
    USUBJID, AVISITN, AVISIT, PARAMCD, PARAM, AVAL, BASE, CHG, starts_with("ANL"),
    ABLFL, APOBLFL,
    VSSTAT
  ) %>%
  mutate(invsdata = "Y") %>%
...
vs_precision <- tidytlg:::make_precision_data(df = advs00, decimal = 4, precisionby = "PARAMCD", precisionon = "AVAL")
...
filtered_advs_timepoints <- unique(advs00 %>%
  filter(AVISIT %in% selvisit) %>%
...
filtered_advs$PARAM <- factor(as.character(filtered_advs$PARAM),
  levels = sel_param,
...
lyt <- basic_table(show_colcounts = FALSE, colcount_format = "N=xx") %>%
  ...
  split_rows_by("PARAM", label_pos = "topleft", split_label = "Blood Pressure", section_div = " ", split_fun = drop_split_levels) %>%
  ...
  analyze("STUDYID", afun = a_summarize_aval_chg_diff_j, 
