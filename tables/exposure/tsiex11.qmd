...
core_ex_vars <- c("USUBJID", "AACTPR", "AACTDU", "AADJ", "AADJP", "AVISIT", "AVISITN" )
...
adex1c$XXX <- factor(adex1c$XXX, levels = c("Prior to infusion", "During infusion"))
...
adex2c$XXX <- factor(adex2c$XXX, levels = c("Prior to infusion", "During infusion"))
...
aact_levels <- c("DOSE REDUCED COMPARED TO PRIOR INFUSION", 
...
adex$AACTx <- factor(as.character(adex$AACTx), levels = aact_levels, labels = aact_labels)
...
lyt <- lyt %>%
  ...
  split_rows_by("XXX",
                split_fun  = keep_split_levels(c("Prior to infusion"
                                                 , "During infusion"
                                                 )),
                section_div = " ",
                child_labels = "visible"
  ) %>%
  summarize_row_groups("XXX" ,
                       cfun = a_freq_j,
