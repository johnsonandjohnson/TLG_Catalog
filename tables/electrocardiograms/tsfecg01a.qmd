...
selvisit <- c("Baseline", "Month 1", "Month 3", "Month 6", "Month 9", "Month 12", "Month 15", "Month 18", "Month 24")
...
eg_precision <- tidytlg:::make_precision_data(df = adeg00, decimal = 3, precisionby = "PARAMCD", precisionon = "AVAL")
...
filtered_adeg_timepoints <- unique(adeg00 %>%
  filter(AVISIT %in% selvisit) %>%
...
lyt_0 <- basic_table(show_colcounts = FALSE, colcount_format = "N=xx") %>%
  ...
  split_rows_by("PARAM", label_pos = "topleft", split_label = obj_label(filtered_adeg$PARAM), section_div = " ", split_fun = drop_split_levels, indent_mod = indent_adj) %>%
